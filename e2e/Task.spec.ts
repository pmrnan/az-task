...
test("タスク追加・移動・削除", async ({ page }) => {
  ...
  const task = page
    .locator("div")
    .filter({ hasText: /^e2eテストタスク優先度:期日:2025\/06\/28$/ })
    .first();
  await expect(task).toBeVisible();
  // 詳細ダイアログ確認
  await task.click();
  await expect(
    page.getByRole("dialog", { name: "e2eテストタスク" }),
  ).toBeVisible();
  await expect(
    page.getByLabel("e2eテストタスク").getByText("高"),
  ).toBeVisible();
  await expect(
    page.getByLabel("e2eテストタスク").getByText("2025/06/28"),
  ).toBeVisible();
  await expect(
    page.getByLabel("e2eテストタスク").getByText("未着手"),
  ).toBeVisible();
  ...
  await task.click();
  await expect(
    page.getByLabel("e2eテストタスク").getByText("着手中"),
  ).toBeVisible();
  await page.getByRole("dialog", { name: "タスク" }).locator("span").click();
  ...
  await page
    .locator("div")
    .filter({ hasText: /^e2eテストタスク$/ })
    .locator("span")
    .click();
  await expect(
    page.getByRole("alertdialog", { name: "タスクを削除しますか？" }),
  ).toBeVisible();
